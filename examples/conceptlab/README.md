...
lab web UI generated from a data-center fabric diagram, then published under
[`docs/examples/conceptlab/output/`](../../docs/examples/conceptlab/output/)
so GitHub Pages can serve the static UI.
...
4. **Compile the lab package.** Composing the semantic-lab framework with
   ...
   rendered FRR/Alpine bootstrap, and emitted the files now in `output/`:
...
5. **Publish the snapshot.** The compiled `output/` tree was copied to
   `docs/examples/conceptlab/output/` so the GitHub Pages site can host the
   UI without running the compiler on every visit.

## Open the example

The public image opens the compiled lab bundled in
[`docs/examples/conceptlab/output/`](../../docs/examples/conceptlab/output/).
