...
- **Front matter** (optional): a block delimited by lines containing only `---` at the very top of the file, with simple `key: value` pairs. Recognized keys: `title`, `theme`, `author`, `header`, `footer`, `footer-image`, `footer-image-alt`, `data`.
- **Header / footer layout** (optional): set `header` and/or `footer` in the front matter to give every slide a consistent framed layout — a **10%** header band (slide's own title on the left, the `header` text on the right), an **80%** content area, and a **10%** footer band (the `footer` text on the left, the slide number `n / total` on the right). The slide's first heading is lifted into the header band so it isn't duplicated in the body. Optional **`footer-image`** (path to a git-tracked PNG/SVG) renders a logo left of the footer text and is embedded at compile time; **`footer-image-alt`** sets the `<img alt>`. If `header`, `footer`, and `footer-image` are all unset, slides use the default full-bleed layout.
