...
| **Framework vs content** | `netlab-fw.xp` owns templates, transforms, and capabilities; `topology.xp` owns the lab instance. |
...
| Path | Role |
|------|------|
| `filters/` | Compile-time Jinja filters (pass the directory; modules may import each other). |
| `netlab-fw.xp` | Framework: template roots, `containerlab.create` / `deploy` / `destroy`, defaults. |
...
Compose sources in order: **filters → framework → topology** (later overrides
earlier; filters must load before transforms that call them).
...
node skills/xform/scripts/xform.min.js \
  docs/examples/xform-netlab/filters \
  docs/examples/xform-netlab/netlab-fw.xp \
...
- uses: ./actions/xform
  with:
    sources: docs/examples/xform-netlab/filters docs/examples/xform-netlab/netlab-fw.xp docs/examples/xform-netlab/topology.xp
...
| Path | Content |
|------|---------|
| `output/netlab-fw.json` | Full compile tree (named after the first `.xp`) |
